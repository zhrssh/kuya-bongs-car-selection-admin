...
import { 
  Search, Plus, Table, Grid as GridIcon, Sparkles, 
  Trash2, Edit, ExternalLink, SlidersHorizontal, 
  RotateCcw, ShieldCheck, Mail, Phone, MapPin, Gauge, X,
  Milestone, Fuel, Upload
} from 'lucide-react';
...
export default function InventoryCMS({
  ...
  const [statusTab, setStatusTab] = useState<'available' | 'sold' | 'archived'>('available');
  ...
  const uniqueMakes = useMemo(() => Array.from(new Set(cars.map(c => c.make))), [cars]);
  const uniqueBodyTypes = ['SUV', 'Sedan', 'Coupe', 'Truck', 'Hatchback', 'Convertible'];
  ...
  const modelsForMake = useMemo(() => {
    ...
    return Array.from(new Set(cars.filter(c => c.make === filters.make).map(c => c.model)));
  }, [cars, filters.make]);
  ...
  const generateRandomCar = () => {
    const brands = [
      { make: 'Tesla', model: 'Model Y', fuelType: 'Electric', bodyType: 'SUV', engine: 'Dual Motor AWD', drivetrain: 'AWD', features: ['Autopilot', 'Supercharger access', 'Premium audio', 'Cold weather pack'] },
      { make: 'BMW', model: 'i4 Gran Coupe', fuelType: 'Electric', bodyType: 'Sedan', engine: 'eDrive40 Single Motor', drivetrain: 'RWD', features: ['BMW Curved Display', 'Parking Assistant', 'Harman Kardon Audio'] },
      { make: 'Porsche', model: 'Taycan', fuelType: 'Electric', bodyType: 'Sedan', engine: 'Dual Electric Motors', drivetrain: 'AWD', features: ['Performance Battery Plus', 'PCCB brakes', 'Chrono clock', 'Passenger screen'] },
      { make: 'Lexus', model: 'RX450h', fuelType: 'Hybrid', bodyType: 'SUV', engine: '3.5L V6 Hybrid', drivetrain: 'AWD', features: ['Mark Levinson Sound', 'Lexus Safety System+', 'Heated/Ventilated Seats'] },
      { make: 'Jeep', model: 'Wrangler Rubicon', fuelType: 'Gasoline', bodyType: 'SUV', engine: '2.0L Turbo I4', drivetrain: '4WD', features: ['Fox Shocks', 'Rock Trac extreme 4WD', 'Removable hard top', 'Warn Winch'] },
      { make: 'Audi', model: 'RS e-tron GT', fuelType: 'Electric', bodyType: 'Coupe', engine: 'Dual Motor Quattro', drivetrain: 'AWD', features: ['Matrix LED headlights', 'Carbon Ceramic Brakes', 'B&O 3D Audiosystem'] },
      { make: 'Rivan', model: 'R1T', fuelType: 'Electric', bodyType: 'Truck', engine: 'Quad-Motor AWD', drivetrain: 'AWD', features: ['Gear Tunnel', 'Camp Kitchen Capable', 'Adaptive Air Suspension', 'Panorail sunroof'] }
    ];
    ...
    const pickedPrice = prices[Math.floor(Math.random() * prices.length)] - Math.floor(Math.random() * 1900);
    ...
    const randomCar: Car = {
      ...
      exteriorColor: ['Midnight Metallic Gray', 'Chalk', 'Satin Frozen Black', 'Tanzanite Blue', 'Acid Green'][Math.floor(Math.random() * 5)],
      interiorColor: ['Black Nappa Leather', 'Merino Truffle', 'Chalk / Bordeaux Red', 'Sustainable Micro-Fiber'][Math.floor(Math.random() * 4)],
      engine: pick.engine,
      ...
      history: {
        ...
        accidents: 0,
        serviceHistory: 'Full digital dealer stamps. Under factory maintenance.'
      },
      seller: {
        name: ['Cassandra Pierce', 'Maxwell Vance', 'Nathaniel Reed', 'Elara Vance'][Math.floor(Math.random() * 4)],
        ...
        location: ['San Francisco, CA', 'New York, NY', 'Austin, TX', 'Miami, FL', 'Seattle, WA'][Math.floor(Math.random() * 5)]
      }
    };
  ...
  const handleFormSubmit = (e: FormEvent) => {
    ...
    const { make, model, year, price, mileage, exteriorColor, interiorColor, description, condition } = formData;

    if (!make || !model || !year || !price || !mileage || !exteriorColor || !description) {
      setFormError('Please fill out all required fields (Brand, Model, Year, Price, Mileage, Exterior, and Description).');
      return;
    }
    ...
    return cars
      .filter(car => {
        ...
        if (filters.searchQuery) {
          ...
          if (!matchMake && !matchModel && !matchDesc && !matchFeatures) return false;
        }

        // Filter by Tab (available, sold, archived)
        const carStatus = car.status || 'available';
        if (statusTab === 'available' && carStatus !== 'available') return false;
      ...
      .sort((a, b) => {
        switch (sortKey) {
          ...
          case 'relevance': default: return b.year - a.year; // default to newer
        }
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        {/* Listings Section */}
        <div className={isFilterSidebarOpen ? "lg:col-span-3 space-y-6" : "lg:col-span-4 space-y-6"}>
          ...
          <div className="flex border-b border-slate-200/80 gap-6 overflow-x-auto pb-0.5">
            ...
            >
              ...
              <span className={`text-[11px] px-2 py-0.5 rounded-full font-mono font-bold ${
                statusTab === 'available' ? 'bg-blue-105 bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600'
              }`}>
                {counts.available}
              </span>
              {statusTab === 'available' && (
            ...
            >
              ...
              <span className={`text-[11px] px-2 py-0.5 rounded-full font-mono font-bold ${
                statusTab === 'sold' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
              }`}>
                {counts.sold}
              </span>
              {statusTab === 'sold' && (
            ...
            >
              ...
              <span className={`text-[11px] px-2 py-0.5 rounded-full font-mono font-bold ${
                statusTab === 'archived' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'
              }`}>
                {counts.archived}
              </span>
              {statusTab === 'archived' && (
      ...
      {filteredAndSortedCars.length === 0 ? (
        <div className="bg-white border border-zinc-200 p-12 text-center rounded-xl shadow-xs">
          <p className="text-zinc-500 text-sm font-medium">No vehicles match your active filtering variables.</p>
          <button 
            onClick={resetFilters} 
                ...
                <div className="relative aspect-[4/3] overflow-hidden bg-slate-50">
                  ...
                  {car.status && car.status !== 'available' && (
                    <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-[1px] flex items-center justify-center pointer-events-none transition-all duration-300">
                      <span className={`px-4 py-2 rounded-xl text-xs font-bold font-mono tracking-wider uppercase border shadow-md transform -skew-x-6 scale-110 select-none ${
                        car.status === 'sold'
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse font-sans">
              ...
              <tbody className="divide-y divide-zinc-100 text-sm text-zinc-700">
                {filteredAndSortedCars.map((car) => (
                  <tr key={car.id} className="hover:bg-zinc-50/40 transition">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                          <div className="font-semibold text-zinc-900">{car.make} {car.model}</div>
                          <div className="text-[11px] text-zinc-500 font-mono">Mfg: {car.year} • {car.transmission}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Body Style */}
                    <td className="p-4">
                      <span className="text-xs font-mono text-zinc-650">{car.bodyType}</span>
                    </td>

                    {/* Condition */}
                    <td className="p-4">
                      <span className={`text-[10px] font-semibold tracking-wider px-2 py-0.5 rounded font-mono uppercase ${
                        car.condition === 'Excellent' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200/60' :
                        car.condition === 'Very Good' ? 'bg-blue-50 text-blue-700 border border-blue-200/60' :
                    ...
                    {/* Status */}
                    <td className="p-4">
                      <span className={`text-[10px] font-bold tracking-wider px-2.5 py-0.5 rounded-full font-mono uppercase border ${
                        (!car.status || car.status === 'available') 
                    ...
                    {/* Fuel Mechanism */}
                    <td className="p-4">
                      <span className="text-xs text-zinc-600 font-mono">{car.fuelType}</span>
                    </td>
        ...
        </div> {/* Listings Section Container end */}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 py-4 px-6 z-40 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] transition-all animate-in slide-in-from-bottom duration-300">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-semibold text-sm text-slate-800">Vehicles selected to compare (Max 3)</h3>
            </div>
            ...
            <div className="flex flex-wrap gap-3 items-center">
              {compareIds.map(id => {
                ...
                return (
                  <div key={id} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-full pl-2 pr-3 py-1 text-xs text-slate-700">
                    <img src={targetCar.imageUrl} alt="preview" className="w-8 h-6 rounded-full object-cover" />
                    <span className="font-medium">{targetCar.make} {targetCar.model}</span>
                    <button 
        ...
        <div className="fixed inset-0 bg-zinc-950/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-zinc-200 text-zinc-800 rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 relative flex flex-col gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-1">Direct feature matrix and specification breakdown for your selected vehicles</p>
            </div>
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-zinc-100">
                    <th className="py-3 px-4 font-semibold text-slate-400 uppercase tracking-wider w-1/4">Specs Matrix</th>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return (
                        <th key={id} className="py-3 px-4 w-1/4 text-center">
                          <img src={item.imageUrl} alt={item.model} className="w-full h-32 object-cover rounded-xl border border-zinc-200 mb-2" />
                          <div className="font-display font-semibold text-sm text-zinc-900">{item.make} {item.model}</div>
                          <div className="text-[10px] text-zinc-405 text-slate-400 font-medium">{item.year} Model</div>
                        </th>
                      );
                    })}
                  </tr>
                ...
                <tbody className="divide-y divide-zinc-100">
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Sale Price</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return <td key={id} className="py-3 px-4 text-center font-bold text-blue-600 text-sm">₱{item.price.toLocaleString()}</td>;
                    })}
                  </tr>
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Odometer Mileage</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return <td key={id} className="py-3 px-4 text-center">{item.mileage.toLocaleString()} km</td>;
                    })}
                  </tr>
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Condition</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return (
                        <td key={id} className="py-3 px-4 text-center">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase border ${getConditionColor(item.condition)}`}>
                            {item.condition}
                          </span>
                        </td>
                      );
                  ...
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Engine / Powertrain</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return <td key={id} className="py-3 px-4 text-center font-medium">{item.engine}</td>;
                    })}
                  </tr>
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Fuel Mechanism</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return <td key={id} className="py-3 px-4 text-center">{item.fuelType}</td>;
                    })}
                  </tr>
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Gearbox Mechanism</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return <td key={id} className="py-3 px-4 text-center">{item.transmission}</td>;
                    })}
                  </tr>
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Drivetrain Wheelbase</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return <td key={id} className="py-3 px-4 text-center">{item.drivetrain}</td>;
                    })}
                  </tr>
                  <tr className="hover:bg-zinc-50/50">
                    <td className="py-3 px-4 font-semibold text-zinc-500">Highlights & Amenities</td>
                    {compareIds.map(id => {
                      const item = cars.find(c => c.id === id);
                      if (!item) return null;
                      return (
                        <td key={id} className="py-3 px-4">
                          <div className="flex flex-wrap justify-center gap-1">
                            {item.features.map((feat, index) => (
                              <span key={index} className="bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 text-[9px] text-slate-600">
                                {feat}
                              </span>
                            ))}
                          </div>
          ...
          <div className="bg-white border border-zinc-200 text-zinc-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl p-6 relative flex flex-col justify-between space-y-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-zinc-900 font-sans tracking-tight flex items-center gap-2">
                ...
                {editingCar ? 'Update Car Listing CMS' : 'Register New Vehicle in Stock'}
              </h3>
              <p className="text-xs text-zinc-450 mt-1">Fill in critical catalog fields to populate filters and real-time statistics charts</p>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Brand / Make *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Model *</label>
                  <input
                    type="text"
                    required
              ...
              </div>                {/* Year, Price, Mileage */}
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Year *</label>
                  <input
                    type="number"
                    required
                    placeholder="2022"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Price (₱ PHP) *</label>
                  <input
                    type="number"
                    required
                    step={1000}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Mileage (km) *</label>
                  <input
                    type="number"
                    required
                    step={1000}
                    ...
                    onChange={(e) => setFormData(p => ({ ...p, mileage: Number(e.target.value) }))}
                    className="w-full bg-zinc-50 border border-zinc-200 rounded px-3 py-2 text-sm text-zinc-800 focus:outline-none focus:border-zinc-300 font-mono"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-500">Body Type</label>
                  <select
                    value={formData.bodyType || 'Sedan'}
                    onChange={(e) => setFormData(p => ({ ...p, bodyType: e.target.value as any }))}
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData(p => ({ ...p, transmission: e.target.value as any }))}
                    className="w-full bg-zinc-50 border border-zinc-200 rounded px-3 py-2 text-sm text-zinc-800 focus:outline-none"
                  >
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-blue-600 font-mono">Status</label>
                  <select
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Exterior Color *</label>
                  <input
                    type="text"
                    required
                    placeholder="Chalk Red"
                    value={formData.exteriorColor || ''}
                    onChange={(e) => setFormData(p => ({ ...p, exteriorColor: e.target.value }))}
                  ...
                  <label className="text-xs font-medium text-zinc-550">Interior Color</label>
                  <input
                    type="text"
                    placeholder="Black Leather"
                    value={formData.interiorColor || ''}
                    onChange={(e) => setFormData(p => ({ ...p, interiorColor: e.target.value }))}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Engine Block</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-550">Drivetrain</label>
                  <input
                    type="text"
                    placeholder="AWD"
                    value={formData.drivetrain || ''}
                    onChange={(e) => setFormData(p => ({ ...p, drivetrain: e.target.value }))}
                    className="w-full bg-zinc-50 border border-zinc-200 rounded px-3 py-2 text-sm text-zinc-800 focus:outline-none focus:border-zinc-300 placeholder-zinc-400"
                  />
                </div>
              ...
              <div className="space-y-2 border border-slate-200 rounded-xl p-4 bg-slate-50/50">
                <label className="text-xs font-semibold text-zinc-700 block">Vehicle Media Image *</label>
                    ...
                    if (file && file.type.startsWith('image/')) {
                      ...
                      reader.onload = (event) => {
                        if (event.target?.result) {
                          setFormData(p => ({ ...p, imageUrl: event.target!.result as string }));
                        }
                      };
                ...
                >
                  ...
                  {formData.imageUrl ? (
                    <div className="relative w-full aspect-video md:max-h-40 rounded-lg overflow-hidden bg-zinc-100">
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <span className="text-xs text-zinc-500"> or drag and drop a file</span>
                      </div>
                ...
                <div className="space-y-1 mt-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider font-mono">Or use direct image URL:</span>
                    {formData.imageUrl && (
                      <button 
                        type="button"
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-zinc-200 rounded px-3 py-1.5 text-xs text-zinc-800 placeholder-zinc-400 focus:outline-none focus:border-zinc-300 font-mono"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-zinc-550">Key Features (comma separated)</label>
                <input
                  type="text"
              ...
              {/* Listing description */}
              <div className="space-y-1">
                <label className="text-xs font-medium text-zinc-550">Overview Description *</label>
                <textarea
                  rows={3}
                  required
              ...
              <div className="border border-zinc-200 p-4 rounded-lg bg-zinc-50 space-y-3">
                <h5 className="text-[10px] uppercase tracking-wider font-semibold text-zinc-450 font-mono">Seller Assignment</h5>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="space-y-0.5">
                    <span className="text-[10px] text-zinc-400 font-medium">Contact Person</span>
                    <input
                      type="text"
                      placeholder="Name"
                      ...
                      onChange={(e) => setFormData(p => ({
                        ...p,
                        seller: { ...(p.seller || { name: '', phone: '', email: '', rating: 5, location: '' }), name: e.target.value }
                      }))}
                  ...
                  <div className="space-y-0.5">
                    <span className="text-[10px] text-zinc-400 font-medium">Contact Mobile</span>
                    <input
                      type="text"
                      placeholder="Phone"
                      ...
                      onChange={(e) => setFormData(p => ({
                        ...p,
                        seller: { ...(p.seller || { name: '', phone: '', email: '', rating: 5, location: '' }), phone: e.target.value }
                      }))}
                      className="w-full bg-white border border-zinc-200 rounded px-2.5 py-1.5 text-xs text-zinc-800 placeholder-zinc-400 focus:outline-none focus:border-zinc-305"
                    />
                  </div>
                  <div className="space-y-0.5">
                    <span className="text-[10px] text-zinc-400 font-medium">Sales Email</span>
                    <input
                      type="email"
                      placeholder="Email"
                      ...
                      onChange={(e) => setFormData(p => ({
                        ...p,
                        seller: { ...(p.seller || { name: '', phone: '', email: '', rating: 5, location: '' }), email: e.target.value }
                      }))}
                      className="w-full bg-white border border-zinc-200 rounded px-2.5 py-1.5 text-xs text-zinc-800 placeholder-zinc-400 focus:outline-none focus:border-zinc-305"
                    />
                  </div>
